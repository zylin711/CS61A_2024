def shuffle(s):
    ...
    "*** YOUR CODE HERE ***"
    lst = []
    half_len = round(len(s) / 2)
    for i in range(half_len):
        lst.append(s[i])
        lst.append(s[half_len + i])
    return lst
...
def balanced(m):
    ...
    if is_planet(m):
        ...
        end_left, end_right = end(left(m)), end(right(m))
        mass_left, mass_right = total_mass(end_left), total_mass(end_right)
        torque_left = length(left(m)) * mass_left
        torque_right = length(right(m)) * mass_right
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        return label(t)
    for b in branches(t):
        current_sum = max(current_sum, max_path_sum(b))
    return  label(t) + current_sum
